...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.parallelize(WikipediaData.lines.map(data => WikipediaData.parse(data)))
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.filter(wikiArticle => wikiArticle.mentionsLanguage(lang))
      .count().toInt
  }
